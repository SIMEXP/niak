...
def main(args=None):
    ...
    parser.add_argument("--func_hint", default=None, description=(
        'A hint to pick one out of many fmri input. For example' 
        'if the fmri study includes "sub-XX_task-rest-truite_bold.nii.gz"'
        'and "sub-XX_task-rest-a_thing_bold.nii.gz" and you need the "truite" flavor'
        '--fmri_input_filter truite'
        'would do the trick'))
    ...
    if pipeline_name == "Niak_fmri_preprocess":
        pipeline = pyniak.load_pipeline.FmriPreprocess(parsed.file_in,
                                                       parsed.folder_out,
