...
Here is a short description of the fMRI preprocessing pipeline that can be adapted in a publication. You are encouraged to include the script that was used to generate the connectomes as supplementary material of the article. In the perspective of [reproducible research](http://reproducibleresearch.net/index.php/Main_Page) you are also encouraged to make the fMRI database publicly and freely available (see, e.g. [openfmri](http://www.openfmri.org openfmri.org])), such that your results can be fully reproduced by other groups. 
